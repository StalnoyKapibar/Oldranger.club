...
public class TestPageableCommentController {
    ...
    @RequestMapping(path = "/test/{topicId}", method = RequestMethod.GET)
    public String getPageableComments(@PathVariable(value = "topicId") Long topicId,
                                      ...
                                      @PageableDefault(sort = "dateTime", direction = Sort.Direction.DESC) Pageable pageable, Model model) {
        Topic topic = topicService.findById(topicId);
        if (page != null) {
            pageable = PageRequest.of(page,10, Sort.by("dateTime").descending());
        }
